...
// 1. Título do Header (VICTORY / DEAD)
const HeaderPlaceholder = ({ result }: { result: 'victory' | 'defeat' }) => {
    const isVictory = result === 'victory';
    const text = isVictory ? 'VICTORY' : 'DEAD';
    const borderColor = isVictory ? 'border-green-600' : 'border-red-700';
    const textColor = isVictory ? 'text-green-500' : 'text-red-600';

    return (
        <div className={`w-64 mx-auto mb-4 p-2 text-center bg-gray-900 border-4 ${borderColor} rounded-full`}>
            <h2 className={`text-xl font-bold tracking-widest ${textColor}`}>{text}</h2>
        </div>
    );
};

// 2. Título Principal (SURVIVED / DIED)
const MainTitlePlaceholder = ({ result }: { result: 'victory' | 'defeat' }) => {
    const isVictory = result === 'victory';
    const text = isVictory ? 'SURVIVED' : 'DIED';
    const textColor = isVictory ? 'text-yellow-400' : 'text-red-700';

    return (
        <div className="text-center mb-6">
            <h1 className={`text-6xl font-black uppercase drop-shadow-md ${textColor}`}>
                {text}
            </h1>
        </div>
    );
...
const AvatarPlaceholder = ({ result }: { result: 'victory' | 'defeat' }) => {
    ...
    return (
        <div className="relative flex justify-center mb-6">
            {halo}
            <div className="w-40 h-40 bg-gray-800 border-4 border-gray-600 rounded-full flex items-center justify-center text-center p-2">
...
// 4. Painel de Estatísticas (Esquerda)
const StatsPanelPlaceholder = ({ result }: { result: 'victory' | 'defeat' }) => {
    ...
    return (
        <div className={`flex-1 bg-gray-900/80 border-4 ${borderColor} rounded-xl p-4 flex flex-col justify-between min-h-[200px]`}>
...
const RewardsPanelPlaceholder = ({ result }: { result: 'victory' | 'defeat' }) => {
    const isVictory = result === 'victory';
    const borderColor = isVictory ? 'border-green-500' : 'border-gray-600'; // Derrota geralmente é cinza ou vermelho desbotado
    const titleColor = isVictory ? 'text-green-400' : 'text-gray-400';

    const xpText = isVictory ? '+1200' : '-500';
    const xpColor = isVictory ? 'text-green-400' : 'text-red-500';
    const lootText = isVictory ? 'Cool Rick Sunglasses' : 'NO LOOT';
    const lootSubtext = isVictory ? 'NEW COSMETIC UNLOCKED!' : '';

    return (
        <div className={`flex-1 bg-gray-900/80 border-4 ${borderColor} rounded-xl p-4 flex flex-col items-center min-h-[200px]`}>
            <h3 className={`text-center font-bold mb-2 uppercase ${titleColor}`}>Rewards</h3>

            {/* XP Section */}
            <div className="w-full text-center mb-4">
                <p className="text-white text-sm font-bold">XP {isVictory ? 'GAINED' : 'LOST'}:</p>
                <p className={`text-2xl font-black ${xpColor}`}>{xpText}</p>

                {/* Progress Bar Placeholder */}
                <div className="w-full h-4 bg-gray-700 rounded-full mt-1 border border-gray-500 overflow-hidden relative">
                    <div
                        className={`h-full ${isVictory ? 'bg-yellow-400' : 'bg-red-600'}`}
...
const FooterButtons = ({ result }: { result: 'victory' | 'defeat' }) => {
    const isVictory = result === 'victory';
    const btnColor = isVictory ? 'bg-green-600 border-green-400 hover:bg-green-500' : 'bg-red-700 border-red-500 hover:bg-red-600';
...
const ResultPage = () => {
    // Estado temporário para debug e alternância de telas
    ...
    const bgClass = gameResult === 'victory'
        ? 'bg-gradient-to-b from-gray-900 to-green-900'
        ...
        <div className={`min-h-screen flex items-center justify-center p-4 ${bgClass}`}>
            ...
            <div className="fixed top-4 right-4 z-50">
                <button
                    ...
                    className="bg-white text-black text-xs font-bold py-2 px-4 rounded shadow hover:bg-gray-200"
                >
            ...
            <div className="w-full max-w-4xl relative">
                ...
                <div className="bg-gray-800 border-8 border-gray-600 rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden">
