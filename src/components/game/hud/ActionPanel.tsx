...
export function ActionPanel() {
  return (
    <div className="w-full md:w-1/3 flex flex-col-reverse md:flex-col gap-2">
      {/* Game Log */}
      <div className="bg-void-black border-2 border-border-muted p-2 h-24 overflow-y-auto rounded font-mono text-xs text-neon-green">
        <p>{`> Match started.`}</p>
        <p>{`> Round 1 ended.`}</p>
        <p className="text-neon-yellow">{`> Rick Sanchez used Beer.`}</p>
        <p>{`> Ejected shell.`}</p>
      </div>
