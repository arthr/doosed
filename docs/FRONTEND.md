...
body {
  @apply bg-space-black text-white font-pixel overflow-hidden;
...
export const PixelCard = ({ children, className = '', variant = 'default' }: PixelCardProps) => {
  ...
  return (
    <div
      className={`
      relative bg-ui-panel border-4 ${borderColors[variant]} 
      shadow-pixel p-2 rounded-sm ${className}
    `}
    >
      {children}
    </div>
...
export const GameScreen = () => {
  ...
  return (
    <div className="h-screen w-screen bg-space-black relative flex flex-col p-4 overflow-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0 bg-[url('/space-bg.png')] bg-cover opacity-50 z-0" />
